...
| Экспорт TXT | ✅ | ✅ |
| Экспорт Markdown / Obsidian Export | ✅ | ✅ |
| Экспорт PDF/SRT | ❌ | ✅ |
| Лимиты (free tier) | ✅ (10/день, макс. 30 мин) | ✅ |
| Подписки (Stripe/Paddle) | ❌ | ✅ |
| Webhooks / API | ❌ | ✅ |
| Дашборд статистики | ❌ | ✅ |
| История запросов | ✅ | ✅ |
| RSS-фиды по топикам и спикерам | ✅ | ✅ |
| Trending Now (счётчик просмотров + `/trending`) | ✅ | ✅ |
| Многоязычность | ❌ | ❌ (v2.0) |
...
### v1.0 Growth Sprint (No-Auth, Zero-Cost)

Фичи, добавленные после базового MVP — не требуют аутентификации, нулевые AI-косты:

| Фича | Цель | Статус |
|---|---|---|
| **Trending Now** (`/trending`) | Динамическая SEO-страница + сигнал поисковикам | ✅ Реализовано |
| **RSS-фиды** (`/topic/{slug}/feed.xml`, `/speaker/{slug}/feed.xml`) | Трафик из RSS-агрегаторов, обратные ссылки | 🔄 Следующий спринт |
| **Obsidian/Markdown Export** (`GET /api/transcribe/{id}/export/md`) | PKM-аудитория (студенты, разработчики) | 🔄 Следующий спринт |

### v2.0+ Backlog (требует Auth / Stripe)

Эти фичи целенаправленно отложены до запуска системы аккаунтов и оплаты:

| Фича | Зависимость |
|---|---|
| Реферальная программа | Auth |
| Коллекции / Закладки | Auth |
| Личные заметки и хайлайты на транскрипте | Auth |
| Weekly Digest Email | Auth + Email |
| Командные рабочие пространства (Teams) | Auth + Stripe |
| AI Quiz (генерация вопросов) | +1 LLM вызов → платный tier |
| YouTube Shorts | Намеренно отклонено: «Save 60 seconds» не ценность |
